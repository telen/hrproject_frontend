...
export default modelExtend(pageModel, {
  namespace: 'inspectionInprogress',
  ...
  subscriptions: {
    setup ({ dispatch, history }) {
      history.listen((location) => {
        if (location.pathname === '/inspection/inspectionInprogress') {
          dispatch({
            type: 'query',
  ...
  effects: {
    ...
    * query ({ payload = {} }, { call, put }) {
      const data = yield call(query, { ...payload, ...{ stage: 1 } })
      if (data.code === '000000') {
        yield put({
          type: 'querySuccess',
          payload: {
    ...
    * update ({ payload }, { select, call, put }) {
      const data = yield call(update, { ...payload, ...{ stage: 1 } })
      if (data.code === '000000') {
        yield put({ type: 'hideModal' })
        yield put({ type: 'query' })
      } else {
